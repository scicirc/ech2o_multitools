...
def trajs(Config, Opti):
    ...
    Opti.nr = 10  # np.round(int(Config.ncpu))
...
def ee(Config, Obs, Opti):
    ...
    if(Config.MSspace == 'trajectory'):
        ...
        with open(Config.FILE_EE+'.EE.Traj' +
                  Config.numsim+'.bias.txt', 'w') as f_out:
            ...
            for i in range(Opti.nvar):
                f_out.write(Opti.names[i]+',' +
                            ','.join([str(bias_ee_tot[i, j]) for j in
                                      range(numObs)])+'\n')
        exit
        with open(Config.FILE_EE+'.EE.Traj'+Config.numsim +
                  '.RMSE.txt', 'w') as f_out:
            f_out.write('Parameter'+','+','.join([outObs[j] for j in
                                                  range(numObs)])+'\n')
            for i in range(Opti.nvar):
    ...
    if(Config.MSspace == 'radial'):
        ...
        with open(Config.FILE_EE+'.EE.RadP'+Config.numsim +
                  '.bias.txt', 'w') as f_out:
            f_out.write('Parameter'+','+','.join([outObs[j] for j in
                                                  range(numObs)])+'\n')
            for i in range(Opti.nvar):
